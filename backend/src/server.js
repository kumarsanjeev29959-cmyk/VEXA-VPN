...
function persist() { saveState({ servers, devices, allocations }); }
function json(res, status, body) {
  const payload = JSON.stringify(body);
  res.writeHead(status, {'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store','X-Content-Type-Options':'nosniff','Content-Length':Buffer.byteLength(payload)});
  res.end(payload);
}
...
function allowRate(req,res){
  const key=`${req.socket.remoteAddress||'unknown'}:${req.url.split('?')[0]}`; const now=Date.now();
