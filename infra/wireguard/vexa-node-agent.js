...
function request(path, method = 'GET', body) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request({
      ...
      method,
      headers: { 'Accept': 'application/json', 'X-VEXA-Node-Token': NODE_TOKEN, ...(body ? {'Content-Type':'application/json'} : {}) },
    }, res => {
      let data = '';
      res.setEncoding('utf8');
      res.on('data', chunk => { data += chunk; });
      res.on('end', () => {
        if (res.statusCode < 200 || res.statusCode >= 300) return reject(new Error(`API HTTP ${res.statusCode}`));
