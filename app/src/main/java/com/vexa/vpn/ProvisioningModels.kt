...
data class VpnServer(val id: String, val name: String, val countryCode: String, val city: String, val hostname: String, val port: Int, val protocol: String, val premium: Boolean, val healthy: Boolean, val loadPercent: Int, val latencyMs: Int?)
data class ProvisioningRequest(val deviceId: String, val publicKey: String, val serverId: String? = null, val fastest: Boolean = true)
data class DeviceProvisioningResponse(val deviceToken: String, val deviceId: String, val expiresAt: String)
data class VpnPeerConfig(val serverPublicKey: String, val address: String, val dns: String, val allowedIPs: String, val persistentKeepalive: Int = 25)
data class VpnConfigResponse(val server: VpnServer, val peer: VpnPeerConfig, val expiresAt: String)
data class ServerSelection(val server: VpnServer, val reason: String)
